...
export async function updateDraftVideoData(updatedVideoData: Partial<Video>) {
    try {
        ...
        const q = query(collection(db, "draft-videos"), where("id", "==", updatedVideoData.id));
        ...
        await updateDoc(doc(docSnapshot.ref.firestore, "draft-videos", docSnapshot.id), updatedVideoData);
    } catch (e) {
